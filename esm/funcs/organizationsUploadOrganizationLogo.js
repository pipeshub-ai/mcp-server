 ...
 * </ul>
 */
export function organizationsUploadOrganizationLogo(client$, request, options) {
    return new APIPromise($do(client$, request, options));
}
async function $do(client$, request, options) {
    const parsed$ = safeParse(request, (value$) => UploadOrganizationLogoRequest$zodSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
    ...
    if (isBlobLike(payload$.logo)) {
        ...
        const contentType = getContentTypeFromFileName(payload$.logo.fileName)
            || "application/octet-stream";
        appendForm(body$, "logo", new Blob([payload$.logo.content], { type: contentType }), payload$.logo.fileName);
    }
