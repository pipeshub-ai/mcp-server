...
import { QueryServiceSearchOpServerList, QueryServiceSearchRequest$zodSchema, } from "../models/queryservicesearchop.js";
 ...
 * </ol>
 */
export function queryServiceQueryServiceSearch(client$, request, options) {
    return new APIPromise($do(client$, request, options));
}
async function $do(client$, request, options) {
    const parsed$ = safeParse(request, (value$) => QueryServiceSearchRequest$zodSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
    ...
    const baseURL$ = options?.serverURL
        || pathToFunc(QueryServiceSearchOpServerList[0], {
            charEncoding: "percent",
        })();
    const path$ = pathToFunc("/query/api/v1/search")();
