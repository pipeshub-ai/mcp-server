 ...
 */
export function documentUploadUploadDocument(client$, request, options) {
    return new APIPromise($do(client$, request, options));
}
async function $do(client$, request, options) {
    const parsed$ = safeParse(request, (value$) => UploadDocumentRequest$zodSchema.parse(value$), "Input validation failed");
    if (!parsed$.ok) {
    ...
    if (isBlobLike(payload$.file)) {
        ...
        const contentType = getContentTypeFromFileName(payload$.file.fileName)
            || "application/octet-stream";
        appendForm(body$, "file", new Blob([payload$.file.content], { type: contentType }), payload$.file.fileName);
    }
