/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { storageConfigurationsGet } from "../../funcs/storageConfigurationsGet.js";
import { formatResult } from "../tools.js";
export const tool$storageConfigurationsGet = {
    name: "storage-configurations-get",
    description: `Get current storage configuration

Retrieve the current storage backend configuration. Returns the configuration for whichever storage type is currently active (Local, S3, or Azure Blob).`,
    annotations: {
        "title": "",
        "destructiveHint": false,
        "idempotentHint": false,
        "openWorldHint": false,
        "readOnlyHint": true,
    },
    tool: async (client, ctx) => {
        const [result] = await storageConfigurationsGet(client, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        return formatResult(result.value);
    },
};
//# sourceMappingURL=storageConfigurationsGet.js.map