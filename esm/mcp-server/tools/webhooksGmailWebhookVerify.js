/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { webhooksGmailWebhookVerify } from "../../funcs/webhooksGmailWebhookVerify.js";
import { formatResult } from "../tools.js";
export const tool$webhooksGmailWebhookVerify = {
    name: "webhooks-gmail-webhook-verify",
    description: `[Connector Service] Gmail webhook verification

⚠️ <b>INTERNAL SERVICE ENDPOINT</b><br><br>

GET handler for Gmail Pub/Sub webhook verification.<br><br>

<b>Service:</b> Connector Service<br>
<b>Port:</b> 8088<br>
<b>Base URL:</b> <code>http://localhost:8088</code><br><br>

<b>Authentication:</b> None required - uses Google Pub/Sub verification<br><br>

<b>Note:</b> Google Pub/Sub may send GET requests for subscription verification.
`,
    annotations: {
        "title": "",
        "destructiveHint": false,
        "idempotentHint": false,
        "openWorldHint": false,
        "readOnlyHint": true,
    },
    tool: async (client, ctx) => {
        const [result] = await webhooksGmailWebhookVerify(client, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        return formatResult(result.value);
    },
};
//# sourceMappingURL=webhooksGmailWebhookVerify.js.map