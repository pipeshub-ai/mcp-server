/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */
import { userGroupsGetUserGroupStats } from "../../funcs/userGroupsGetUserGroupStats.js";
import { formatResult } from "../tools.js";
export const tool$userGroupsGetUserGroupStats = {
    name: "user-groups-get-user-group-stats",
    description: `Get user group statistics

Retrieve statistics for all user groups in the organization.<br><br>
<b>Statistics Include:</b><br>
<ul>
<li>Total number of groups by type</li>
<li>Member counts per group</li>
<li>Active vs deleted groups</li>
<li>Recently created/modified groups</li>
</ul>
<b>Use Cases:</b><br>
<ul>
<li>Admin dashboard displays</li>
<li>Organization analytics</li>
<li>Capacity planning</li>
</ul>
`,
    annotations: {
        "title": "",
        "destructiveHint": false,
        "idempotentHint": false,
        "openWorldHint": false,
        "readOnlyHint": true,
    },
    tool: async (client, ctx) => {
        const [result] = await userGroupsGetUserGroupStats(client, { fetchOptions: { signal: ctx.signal } }).$inspect();
        if (!result.ok) {
            return {
                content: [{ type: "text", text: result.error.message }],
                isError: true,
            };
        }
        return formatResult(result.value);
    },
};
//# sourceMappingURL=userGroupsGetUserGroupStats.js.map